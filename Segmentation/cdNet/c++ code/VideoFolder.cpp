...
VideoFolder::VideoFolder(const string& videoPath, const string& binaryPath, const string& groundtruthFolder)
...
void VideoFolder::setExtension() {
	...
	for (auto it = exts.begin(); it < exts.end(); ++it) {
		const string &ext = *it;
		if (fileExists(binaryPath + "" + toFrameNumber(range.first) + ext)) {
			binaryExtension = ext;
			return;
		}
	}
...
const string VideoFolder::inputFrame(const uint idx) const {
	return videoPath + "input/" + toFrameNumber(idx) + ".jpg";
}

const string VideoFolder::binaryFrame(const uint idx) const {
	return binaryPath + "" + toFrameNumber(idx) + binaryExtension;
}

const string VideoFolder::gtFrame(const uint idx) const {
	return videoPath + groundtruthFolder + "/" + toFrameNumber(idx) + ".png";
}
